        ...
        <div className="space-y-6 animate-fade-in pb-20">
            ...
            <div className="flex flex-col gap-6">
                <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">Gérez vos limites de dépenses avec précision.</p>
                    </div>

                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => setIsTutorialOpen(true)}
                            className="text-slate-500 hover:text-slate-700 px-3 py-2 text-sm font-medium transition-colors flex items-center gap-2"
                        >
                        ...
                        <button
                            ...
                            className="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm"
                        >
                            Catégories
                        </button>
                        <button
                            onClick={() => { setEditingBudgetData(null); setIsBudgetModalOpen(true); }}
                            className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-md flex items-center gap-2"
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="p-6 border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="text-slate-500 text-sm font-medium mb-1">Trésorerie Disponible</div>
                    <div className="text-3xl font-bold text-indigo-600 tracking-tight">{availableFunds !== undefined ? formatAriary(availableFunds) : '-- Ar'}</div>
                    <div className="mt-2 text-xs text-indigo-400 font-medium">Fonds réels disponibles</div>
                </Card>

                <Card className="p-6 border-slate-100 shadow-sm">
                    <div className="text-slate-500 text-sm font-medium mb-1">Budget Alloué</div>
                    <div className="text-3xl font-bold text-slate-900 tracking-tight">{formatAriary(totalPlanned)}</div>
                    <div className="mt-2 text-xs text-slate-400">Total pour cette période</div>
                </Card>

                <Card className="p-6 border-slate-100 shadow-sm">
                    <div className="text-slate-500 text-sm font-medium mb-1">Dépenses Réelles</div>
                    <div className="text-3xl font-bold text-slate-900 tracking-tight">{formatAriary(totalSpent)}</div>
                ...
                <Card className="p-6 border-slate-100 shadow-sm">
                    <div className="text-slate-500 text-sm font-medium mb-1">Reste à dépenser</div>
                    <div className={`text-3xl font-bold tracking-tight ${totalPlanned - totalSpent < 0 ? 'text-red-600' : 'text-emerald-600'}`}>
